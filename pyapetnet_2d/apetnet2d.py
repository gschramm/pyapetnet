...
from tensorflow.keras.layers       import Input, Conv2D, ReLU, PReLU, BatchNormalization, Add, Concatenate
...
def apetnet2d(n_ch               = 2, 
  ...
  inputs = [Input(shape = (None, None,1), name = 'input_' + str(x)) for x in range(n_ch)]
  ...
  if n_ind_layers > 0: 
    ...
    for i in range(n_ind_layers): 
      for j in range(n_ch):
        x1_list[j] = Conv2D(n_kernels_ind, kernel_shape, padding = 'same', kernel_initializer = init_val_ind,
                            name = 'conv2d_ind_' + str(i) + '_' + str(j))(x1_list[j])
  ...
  for i in range(n_common_layers): 
    x1 = Conv2D(n_kernels_common, kernel_shape, padding = 'same', kernel_initializer = init_val,
                name = 'conv2d_' + str(i))(x1)
    x1 = PReLU(shared_axes=[1,2,3], name = 'prelu_' + str(i))(x1)
  
  # layers that adds all features
  x1 = Conv2D(1, (1,1), padding='same', name = 'conv_11', 
