...
def apetnet(
    ...
    inputs = [
        Input(shape=(None, None, None, 1), name="input_" + str(x)) for x in range(n_ch)
    ]
        ...
        for i in range(n_ind_layers):
            for j in range(n_ch):
                x1_list[j] = Conv3D(
                    ...
                    name="conv3d_ind_" + str(i) + "_" + str(j),
                )(x1_list[j])
                if add_batchnorm:
                    x1_list[j] = BatchNormalization(
                        name="batchnorm_ind_" + str(i) + "_" + str(j)
                    )(x1_list[j])
                x1_list[j] = PReLU(
                    shared_axes=[1, 2, 3], name="prelu_ind_" + str(i) + "_" + str(j)
                )(x1_list[j])
        # concatenate inputs
    ...
    x1 = Conv3D(
        ...
        padding="same",
        name="conv_111",
        kernel_initializer=RandomNormal(mean=0.0, stddev=np.sqrt(2)),
    )(x1)
...
def apetnet_vv5_onnx(
    ...
    mri_image = Cropping3D(
        cropping=((ipt_dim_crop, 0), (0, 0), (0, 0)), name="extract_mri"
    )(ipt)
    pet_image = Cropping3D(
        cropping=((0, ipt_dim_crop), (0, 0), (0, 0)), name="extract_pet"
    )(ipt)
    ...
    if not debug:
        ...
        if n_ind_layers > 0:
            init_val_ind = RandomNormal(
                mean=0.0, stddev=np.sqrt(2 / (np.prod(kernel_shape) * n_kernels_ind))
            )
            ...
            for i in range(n_ind_layers):
                pet_image_ind = Conv3D(
                    ...
                    kernel_initializer=init_val_ind,
                )(pet_image_ind)
                pet_image_ind = PReLU(
                    shared_axes=[1, 2, 3], name="prelu_pet_ind_" + str(i)
                )(pet_image_ind)
                ...
                mri_image_ind = PReLU(
                    shared_axes=[1, 2, 3], name="prelu_mri_ind_" + str(i)
                )(mri_image_ind)
        ...
        init_val_common = RandomNormal(
            mean=0.0, stddev=np.sqrt(2 / (np.prod(kernel_shape) * n_kernels_common))
        )
        ...
        for i in range(n_common_layers):
            net = Conv3D(
                ...
                padding="same",
                name="conv3d_" + str(i),
                kernel_initializer=init_val_common,
            )(net)
